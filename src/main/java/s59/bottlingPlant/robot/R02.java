...
public class R02 extends Robot {
    ...
    private void putBottlesInMachine(Bottle[][] bottles) {
        if (bottles == null || bottles.length != bottlingPlant.getEmptyBottlesLane().getLaneContent().length)
            throw new IllegalArgumentException("The amount of bottles to put in the BottlingPlant must be " + bottlingPlant.getEmptyBottlesLane().getLaneContent().length);
